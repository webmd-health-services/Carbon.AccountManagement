...
Describe 'Test-Identity' {
    It 'should find local group' {
        $identity = Test-Identity -Name 'Administrators'
        $identity | Should -Be $true
    }
    
    It 'should find local user' {
        $samAccountName = Test-Identity -Name $user1.SamAccountName 
        $samAccountName | Should -Be $true
    }
    
    if( (Get-WmiObject -Class 'Win32_ComputerSystem').Domain -ne 'WORKGROUP' )
    {
        It 'should find domain user' {
            $identity = Test-Identity -Name ('{0}\Administrator' -f $env:USERDOMAIN)
            $identity | Should -Be $true
        }
    }
