...
Describe ('Carbon.AccountManagement.help topic') {
    It 'should have one' {
        Init
        GivenModuleImported
        ThenHelpTopic 'about_Carbon.AccountManagement' -Exists
    }
}

Describe ('Carbon.AccountManagement.command verbs') {
...
Describe ('Carbon.AccountManagement.command help topics') {
    It 'should have a help topic for each command' {
        Init
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'Carbon.AccountManagement' -CommandType Function,Cmdlet,Filter))
        {
